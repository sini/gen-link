...
# pin source; no library code reads it any more. All 6 dependencies are root inputs of the root
...
# one node. Coherent pins in THIS directory's `flake.lock` resolve to one store path and `import`
# memoises, so there is no second instance for a threading to collapse. What makes the count one is
# now the PINS, and the roster-wide coherence check that keeps them coherent is the hub's rather
# than this file's.
#
