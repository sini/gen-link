...
| Materialization — moving class content into a class evaluation | `gen-view` — "gen-view: the substrate's derived-view constructor — the (L, E, \<, k) carrier with van Antwerpen's relation sort published as a raw calculus, and the named compositions over it". A channel is a named materialized query result, so the terminal move is a `viewRelation` over an authored scope graph and the cell it lands in is a `placement` fact rather than a declaration field. `gen-view` is **not** a gen-link input — `lib/**` reads nothing from it, so the root flake declares no dependency on it. It reaches this repository through `ci/flake.nix:38` alone, as the `genView` specialArg `ci/tests/conductor-oracle.nix` exercises. The released `gen-edge` (S,T,P,M) edge algebra is the surface this replaces at that call site; it still ships and is still the ecosystem's materialization owner until its own retirement lands |
...
All rows re-measured in this run at the revision this file ships with, over a fixture built from `ci/flake.lock` with **one** `gen-schema` instance threaded through `gen-aspects` and `gen-scope` (mirroring the flake `follows` — letting either self-construct its own puts two content-address formulas in one evaluation, and the declared holes then read as unwired). `ks` declares `nixos` as `category = "class"` and `dbcap`/`dbreq` as `category = "facet"`, `contract = "capability"`, each with a declared `merge.types.raw` option; `regA` holds `apps/media/pg` providing `[ "read" "write" ]`; `regB` holds the same path `apps/media/pg` plus `apps/app` requiring `[ "read" ]` and including `keyRef "a/apps/media/pg"`; `srcs` places them at origins `[ "a" ]` / `[ "b" ]`; `wired = link { sources = srcs; wire."b/apps/app".dbreq = "a/apps/media/pg"; }`; `unwired = link { sources = srcs; }`; `T = e: (builtins.tryEval e).success`.
...
| ★ **DISSOLVED, recorded because it shipped as a trap for a while.** `resolved` used to be keyed by the holeless `nodeId` while the bound record carried the instantiation id, so `wired.resolved ? (head wired.bound).id` ⇒ `false` and a consumer joining the two silently got nothing | Both are now the node's identifier: `wired.resolved ? "b/apps/app"` ⇒ `true`, `(head wired.bound).identifier` ⇒ `"b/apps/app"`, and the two agree ⇒ `true`. That is the conflation ADR-0016 ruling 5 forbids, removed rather than documented. Test: `test-resolution-through-gen-resolve` (`ci/tests/link.nix`) |
...
| ★ **STILL LIVE, and the migration did not close it** — a `wire` entry naming a facet the source never declared is ACCEPTED, becomes a relatum, and forks the requirer's identity | `lib/facets.nix:10` answers `"capability"` for any key at all, `:18` returns `[ ]` for an undeclared one, and `lib/contract.nix:27-30` passes `requires = [ ]` against any `provides`. Adding `notAFacet = "a/apps/media/pg"` beside a real `dbreq` filling: the link succeeds ⇒ `true`, the hole rows are `[ "dbreq" "notAFacet" ]`, and the requirer's identity differs from the same wire without it ⇒ `true`. What the migration changed is only that the fork is now VISIBLE as a declared edge. Closing it needs a `holesOf` check at the wire site |
