...
| [gen-algebra](https://github.com/sini/gen-algebra)   | Pure primitives (record algebra, either, intensional identity)                                                                                                                                                                                                                                         |
...
4. **Resolve cross-origin references.** Declare a gen-view `referenceResolution` (the forward `includes` arm, Néron et al. 2015 rule (X)) over the merged graph *as the scope*, injecting that same gen-scope as its query authority, with no boundary marks declared (`marks = _: [ ]`: the federation has no boundary vocabulary, and gen-view requires the field rather than defaulting it). Resolution is active-edge-driven and lazy — gen-link does not scan.
...
The substrate keeps two things apart, and this library used to merge them.
